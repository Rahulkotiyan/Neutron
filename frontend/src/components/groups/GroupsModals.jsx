    ...
    <>
      ...
      {showCreateGroupModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-xl px-4" onClick={resetCreateGroupModal}>
          <div className="bg-[#050505] border border-white/[0.05] text-white w-full max-w-md max-h-[90vh] flex flex-col rounded-[2.5rem] overflow-hidden shadow-[0_0_100px_rgba(255,255,255,0.02)] animate-in fade-in zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
            <div className="flex border-b border-white/[0.03] bg-black/50 shrink-0">
              {['Details', 'Members'].map((label, i) => (
                <button
                  key={label}
              ...
              {createStep === 1 ? (
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full p-4 sm:p-5 bg-black border border-white/[0.05] rounded-2xl focus:border-white focus:ring-1 focus:ring-white/10 outline-none text-sm transition-all text-white placeholder-zinc-800 font-bold tracking-tight"
                      placeholder="e.g. Chess Club"
                    />
                  </div>
                  <div>
                    <label className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em] mb-3 block">Description</label>
                    <textarea
                      value={groupDescription}
                      onChange={e => setGroupDescription(e.target.value)}
                      rows={2}
                      className="w-full p-4 sm:p-5 bg-black border border-white/[0.05] rounded-2xl focus:border-white focus:ring-1 focus:ring-white/10 outline-none text-sm transition-all text-white placeholder-zinc-800 resize-none font-bold tracking-tight"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="flex gap-2">
                        {['CLUB', 'STUDY'].map(type => (
                          <button
                            key={type}
                            type="button"
                            onClick={() => setGroupType && setGroupType(type)}
                            className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] border transition-all ${
                              (groupType || 'CLUB') === type ? 'bg-white text-black border-white' : 'bg-black text-zinc-700 border-white/[0.05] hover:text-white hover:border-white/20'
                            }`}
                          >
                            {type}
                          </button>
                    ...
                    <div>
                      <label className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em] mb-3 block">Who can join?</label>
                      <div className="flex gap-2">
                        {[
                          ...
                          <button
                            key={policy.value}
                            type="button"
                            onClick={() => setJoinPolicy && setJoinPolicy(policy.value)}
                            className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] border transition-all ${
                              (joinPolicy || 'PUBLIC') === policy.value ? 'bg-white text-black border-white' : 'bg-black text-zinc-700 border-white/[0.05] hover:text-white hover:border-white/20'
                            }`}
                          >
                            {policy.label}
                          </button>
                  ...
                  <div>
                    <label className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em] mb-3 block">Who can send messages?</label>
                    <div className="flex gap-2">
                      {[
                        ...
                        <button
                          key={perm.value}
                          type="button"
                          onClick={() => setMessagePermission && setMessagePermission(perm.value)}
                          className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] border transition-all ${
                            (messagePermission || 'everyone') === perm.value ? 'bg-white text-black border-white' : 'bg-black text-zinc-700 border-white/[0.05] hover:text-white hover:border-white/20'
                          }`}
                        >
                          {perm.label}
                        </button>
                  ...
                  <div className="flex justify-between items-center pt-4">
                    <button onClick={resetCreateGroupModal} className="text-zinc-700 hover:text-white text-[10px] font-black uppercase tracking-[0.2em] transition-colors">Cancel</button>
                    <button
                      onClick={() => setCreateStep(2)}
                      disabled={!groupName.trim()}
                      className="bg-white hover:bg-zinc-100 disabled:opacity-10 text-black px-8 py-3 sm:py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] transition-all shadow-xl shadow-white/10 active:scale-95"
                    >
                      Next →
                    </button>
                  </div>
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-600">Members</h2>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-14 pr-5 py-4 sm:py-5 bg-black border border-white/[0.05] rounded-2xl focus:border-white outline-none text-sm transition-all text-white placeholder-zinc-800 font-bold tracking-tight"
                    ...
                    {memberResults.length > 0 && (
                      <div className="absolute top-full left-0 right-0 mt-3 bg-[#080808] border border-white/[0.05] rounded-[1.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.5)] max-h-56 overflow-y-auto z-[110] scrollbar-hide animate-in slide-in-from-top-2">
                         {memberResults.map(u => (
                          <div key={u.id || u._id} onClick={() => addInviteMember(u)} className="flex items-center gap-4 p-4 hover:bg-white/[0.03] cursor-pointer transition-colors border-b border-white/[0.02] last:border-0">
                  ...
                  <div className="flex flex-wrap gap-2 max-h-32 sm:max-h-40 overflow-y-auto scrollbar-hide p-1">
                    {invitedMembers.map(u => (
                      <div key={u.id || u._id} className="flex items-center gap-3 bg-white/[0.03] border border-white/[0.05] pl-4 pr-2 py-2 rounded-xl group transition-all hover:bg-white/[0.05]">
                        ...
                        <div 
                          ...
                          onClick={() => removeInviteMember(u.id || u._id)}
                        >
                          <Xmark size={12} />
                        </div>
                      </div>
                  ...
                  <div className="flex items-center gap-4 bg-white/[0.02] p-4 rounded-2xl border border-white/5">
                    <input
                      ...
                      onChange={e => setAssignAsAdmin && setAssignAsAdmin(e.target.checked)}
                      className="w-5 h-5 rounded-lg bg-black border-white/10 accent-white appearance-none border checked:bg-white transition-all cursor-pointer relative checked:after:content-['✓'] checked:after:absolute checked:after:inset-0 checked:after:flex checked:after:items-center checked:after:justify-center checked:after:text-black checked:after:text-[10px] checked:after:font-black shrink-0"
                    />
                    <label htmlFor="assign-admin" className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-500 cursor-pointer hover:text-zinc-300 transition-colors">Make selected users Admin</label>
                  </div>

                  <div className="flex justify-between items-center pt-4">
                    <button onClick={() => setCreateStep(1)} className="text-zinc-700 hover:text-white text-[10px] font-black uppercase tracking-[0.2em] transition-colors">Back</button>
                     <button
      ...
      {showCreateChannelModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-xl px-4" onClick={() => setShowCreateChannelModal(false)}>
          <div className="bg-[#050505] border border-white/[0.05] text-white w-full max-w-sm rounded-[2.5rem] p-10 shadow-2xl animate-in fade-in zoom-in-95" onClick={e => e.stopPropagation()}>
            <h2 className="text-[10px] font-black uppercase tracking-[0.3em] mb-8 text-zinc-600">Create Channel</h2>
            <form onSubmit={handleCreateChannel} className="space-y-8">
              ...
              <div className="flex gap-4 pt-4">
                 <button type="button" onClick={() => setShowCreateChannelModal(false)} className="flex-1 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-zinc-800 hover:text-white transition-colors">Cancel</button>
      ...
      {showPollModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-xl px-4" onClick={resetPollModal}>
          <div className="bg-[#050505] border border-white/[0.05] text-white w-full max-w-md rounded-[2.5rem] p-10 shadow-2xl overflow-y-auto max-h-[90vh] scrollbar-hide animate-in fade-in zoom-in-95" onClick={e => e.stopPropagation()}>
            <h2 className="text-[10px] font-black uppercase tracking-[0.3em] mb-8 text-zinc-600">Create Poll</h2>
            <div className="space-y-8">
              <div>
                <label className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em] mb-3 block">Question</label>
                <input
                  type="text"
                  value={pollQuestion}
              ...
              <div className="space-y-4">
                <label className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em] block">Options</label>
              ...
              <div className="flex items-center gap-4 bg-white/[0.02] p-4 rounded-2xl border border-white/5">
                <input
                  ...
                  className="w-5 h-5 rounded-lg bg-black border-white/10 accent-white appearance-none border checked:bg-white transition-all cursor-pointer relative checked:after:content-['✓'] checked:after:absolute checked:after:inset-0 checked:after:flex checked:after:items-center checked:after:justify-center checked:after:text-black checked:after:text-[10px] checked:after:font-black shrink-0"
                />
                <label htmlFor="multi-poll" className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-500 cursor-pointer hover:text-zinc-300 transition-colors">Allow multiple answers</label>
              </div>
              <div className="flex gap-4">
                 <button onClick={resetPollModal} className="flex-1 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-zinc-800 hover:text-white transition-colors">Cancel</button>
      ...
      {showInviteModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-xl px-4" onClick={() => setShowInviteModal(false)}>
          <div className="bg-[#050505] border border-white/[0.05] text-white w-full max-w-sm rounded-[2.5rem] p-10 shadow-2xl animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
            <h2 className="text-[10px] font-black uppercase tracking-[0.3em] mb-8 text-zinc-600">Invite Link</h2>
            <div className="space-y-8">
              <p className="text-zinc-700 text-[10px] leading-relaxed font-black uppercase tracking-[0.1em]">Generate a link to invite members to this group.</p>
              {inviteCode ? (
