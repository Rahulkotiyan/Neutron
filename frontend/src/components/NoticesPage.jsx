...
const NoticesPage = ({ isSidebarOpen, currentUser, token }) => {
  ...
  const noticeTypes = ["NOTICE", "ANNOUNCEMENT", "POSTER", "CIRCULAR", "EVENT", "FEST", "ACADEMIC"];
  const categories = ["GENERAL", "ACADEMIC", "PLACEMENT", "FEST", "CLUB", "ADMINISTRATION", "HOSTEL", "OTHERS"];
  ...
  useEffect(() => {
    filterNotices();
  }, [notices, searchTerm, selectedType, selectedCategory, selectedPriority, sortBy]);
  ...
  const filterNotices = () => {
    ...
    if (selectedType !== "ALL") filtered = filtered.filter(n => n.noticeType === selectedType);
  ...
  const handleLike = async (noticeId, e) => {
    ...
    try {
      ...
      setNotices(notices.map(n => n._id === noticeId ? { ...n, likes: response.data.likes } : n));
  ...
  const handleAddComment = async (noticeId) => {
    ...
    try {
      ...
      setSelectedNotice({ ...selectedNotice, comments: response.data.comments });
      setNewComment("");
      // Update main list as well to reflect comment count
      setNotices(notices.map(n => n._id === noticeId ? {...n, comments: response.data.comments} : n));
    } catch (err) {
  ...
  const getTypeIcon = (type) => {
    switch (type) {
      ...
      case "POSTER": return <FileText size={14} />;
      case "CIRCULAR": return <Bell size={14} />;
      case "EVENT": return <Calendar size={14} />;
      case "FEST": return <Zap size={14} />;
  ...
  const hasActiveFilters = selectedType !== "ALL" || selectedCategory !== "ALL" || selectedPriority !== "ALL" || sortBy !== "pinned";

  return (
    <div className="w-full h-full flex-1 overflow-auto bg-black text-zinc-300 relative selection:bg-red-500/30">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <h1 className="text-4xl md:text-5xl font-extrabold text-white tracking-tight leading-tight mb-3">
              Official<br/>Notices Hub
            </h1>
            <p className="text-zinc-400 text-lg max-w-xl">
              Stay in the loop with the latest campus announcements, event posters, circulars, and academic updates.
            </p>
          </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 md:px-8 py-8 flex flex-col xl:flex-row gap-8 items-start">
        ...
        <div className="w-full xl:w-72 xl:sticky xl:top-8 shrink-0 space-y-6">
          ...
          <div className="bg-zinc-900/50 backdrop-blur-md border border-white/5 rounded-3xl p-6 shadow-2xl">
            <div className="flex items-center justify-between xl:mb-6 cursor-pointer xl:cursor-auto" onClick={() => setShowFilters(!showFilters)}>
              ...
              <ChevronDown size={20} className={`xl:hidden text-zinc-500 transition-transform ${showFilters ? 'rotate-180' : ''}`} />
            </div>

            <div className={`space-y-6 xl:block ${showFilters ? 'block pt-6' : 'hidden'}`}>
              
              {/* Filter Selects */}
              {[
                { label: "Notice Type", value: selectedType, setter: setSelectedType, options: noticeTypes, allLabel: "All Types" },
                { label: "Category", value: selectedCategory, setter: setSelectedCategory, options: categories, allLabel: "All Categories" },
                { label: "Priority", value: selectedPriority, setter: setSelectedPriority, options: priorities, allLabel: "All Priorities" },
                { label: "Sort By", value: sortBy, setter: setSortBy, options: ["priority", "views", "likes"], labels: {"priority": "Highest Priority", "views": "Most Viewed", "likes": "Most Liked"}, allLabel: "Pinned First (Default)" }
              ].map((filter, idx) => (
                <div key={idx} className="space-y-2">
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="w-full appearance-none bg-zinc-950/50 hover:bg-zinc-950 border border-white/5 text-sm text-zinc-300 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500/50 transition-all cursor-pointer"
                    >
                      <option value={filter.label === "Sort By" ? "pinned" : "ALL"}>{filter.allLabel}</option>
                      {filter.options.map((opt) => (
                        <option key={opt} value={opt}>{filter.labels ? filter.labels[opt] : opt}</option>
                      ))}
                    </select>
              ...
              {hasActiveFilters && (
                <button 
                  onClick={() => {
                    setSelectedType("ALL"); setSelectedCategory("ALL"); setSelectedPriority("ALL"); setSortBy("pinned"); setSearchTerm("");
                  }}
          ...
          <div className="hidden xl:block text-zinc-500 text-xs font-medium px-4">
             Showing <strong className="text-white">{filteredNotices.length}</strong> notices
          </div>
        </div>
        ...
        <div className="flex-1 w-full">
          {loading ? (
            <div className="flex flex-col items-center justify-center h-64 gap-4">
              ...
              <p className="text-zinc-500 font-medium tracking-tight animate-pulse">Fetching notices...</p>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-20 px-4 text-center border border-white/5 rounded-3xl bg-zinc-900/20 backdrop-blur-sm">
              ...
              <p className="text-zinc-500 max-w-md mx-auto">
                We couldn't find any notices matching your current filters. Try tweaking your search or check back later!
              </p>
              {hasActiveFilters && (
                <button 
                  onClick={() => {
              ...
              {filteredNotices.map((notice) => (
                <div
                  ...
                  className={`group relative flex flex-col bg-zinc-900/40 backdrop-blur-md border rounded-[2rem] overflow-hidden hover:-translate-y-1 transition-all duration-300 cursor-pointer shadow-xl ${
                    notice.pinned ? 'border-red-500/30 bg-red-950/10 hover:border-red-500/60 shadow-red-900/5' : 'border-white/5 hover:border-white/20'
                  }`}
                >
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full overflow-hidden shrink-0 border border-white/10">
                         {notice.publisher?.avatar ? (
                          <img src={notice.publisher.avatar} alt="" className="w-full h-full object-cover" />
                        ) : (
                      ...
                      <div>
                        ...
                        <p className="text-xs text-zinc-500 font-medium">
                          {formatDate(notice.createdAt)} • {notice.views} views
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      {notice.pinned && (
                        <div className="w-8 h-8 rounded-full bg-red-500/10 text-red-500 flex items-center justify-center shrink-0" title="Pinned Notice">
                          <Pin size={16} className="fill-current" />
                        </div>
                      )}
                      {currentUser && notice.publisher && 
                        (
                          // The backend nests the populated object under publisher._id
                          String(notice.publisher?._id?._id || notice.publisher?._id || notice.publisher) === String(currentUser._id || currentUser.id)
                        ) && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                  ...
                  <div className="p-6 pt-0 flex-1 flex flex-col">
                    <div className="flex flex-wrap gap-2 mb-4">
                      <span className={`inline-flex items-center border rounded-full px-2.5 py-0.5 text-[10px] font-bold tracking-wider uppercase ${getPriorityColor(notice.priority)}`}>
                        {notice.priority}
                      </span>
                  ...
                  {notice.posterUrl && (
                    <div className="h-40 w-full bg-zinc-950 overflow-hidden relative">
                      <div className="absolute inset-0 bg-gradient-to-t from-zinc-900 to-transparent z-10"></div>
                      <img src={notice.posterUrl} alt="" className="w-full h-full object-cover opacity-60 group-hover:opacity-100 group-hover:scale-105 transition-all duration-700" />
                    </div>
                  )}
                  ...
                  <div className="px-6 py-4 border-t border-white/5 bg-black/20 flex items-center justify-between">
                    <div className="flex gap-4">
                      <button 
                        ...
                        className={`flex items-center gap-1.5 text-xs font-bold transition-colors ${
                          notice.likes?.includes(currentUser?._id) ? 'text-red-500' : 'text-zinc-500 hover:text-white'
                        }`}
                      >
                        <Heart size={16} className={notice.likes?.includes(currentUser?._id) ? 'fill-current' : ''} />
                    ...
                    {notice.attachments?.length > 0 && (
                      <div className="text-zinc-500" title={`${notice.attachments.length} Attachments`}>
                        <FileText size={16} />
                      </div>
                    )}
                  </div>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 sm:p-6 bg-black/80 backdrop-blur-xl animate-in fade-in duration-300 overflow-y-auto">
          <div className="w-full max-w-4xl bg-zinc-950 border border-white/10 rounded-[2rem] shadow-2xl relative overflow-hidden animate-in zoom-in-95 duration-300 my-auto flex flex-col md:flex-row">
            ...
            <div className="absolute top-4 right-4 flex items-center gap-2 z-20">
              {currentUser && selectedNotice.publisher && 
                (
                  // The backend nests the populated object under publisher._id
                  String(selectedNotice.publisher?._id?._id || selectedNotice.publisher?._id || selectedNotice.publisher) === String(currentUser._id || currentUser.id)
                ) && (
                <button
            ...
            {selectedNotice.posterUrl && (
              <div className="w-full md:w-2/5 md:min-h-[600px] bg-zinc-900 relative">
                <img
            ...
            <div className={`flex-1 p-8 md:p-10 flex flex-col h-full max-h-[85vh] overflow-y-auto ${!selectedNotice.posterUrl ? 'max-w-3xl mx-auto' : ''}`}>
              
              <div className="flex items-center gap-3 mb-6">
                {selectedNotice.publisher?.avatar ? (
                  <img src={selectedNotice.publisher.avatar} alt="" className="w-12 h-12 rounded-full border border-white/10" />
                ) : (
                ...
                <div>
                  <p className="font-bold text-white leading-tight">{selectedNotice.publisher?.name}</p>
                  <p className="text-xs text-zinc-500">{formatDate(selectedNotice.createdAt)} • {selectedNotice.views} views</p>
                </div>
              </div>

              <div className="flex flex-wrap gap-2 mb-4">
                <span className={`inline-flex items-center border rounded-full px-3 py-1 text-[10px] font-bold tracking-wider uppercase ${getPriorityColor(selectedNotice.priority)}`}>
              ...
              {(selectedNotice.eventDate || selectedNotice.location || selectedNotice.contactPerson || selectedNotice.tags?.length > 0) && (
                <div className="bg-white/5 border border-white/10 rounded-2xl p-6 mb-8 space-y-4">
                  <h4 className="text-sm font-bold text-white tracking-tight">Additional Details</h4>
                  ...
                  {selectedNotice.eventDate && (
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-zinc-200 font-medium">{new Date(selectedNotice.eventDate).toLocaleString()}</p>
                      </div>
                    </div>
                  )}
                  ...
                  {selectedNotice.location && (
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-zinc-200 font-medium">{selectedNotice.location}</p>
                      </div>
                    </div>
                  )}
                  ...
                  {selectedNotice.contactPerson && (
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Contact</p>
                        <p className="text-sm text-zinc-200 font-medium whitespace-nowrap">
                          {selectedNotice.contactPerson} {selectedNotice.contactPhone && `• ${selectedNotice.contactPhone}`}
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  {selectedNotice.tags?.length > 0 && (
                    <div className="pt-2 flex flex-wrap gap-2">
                      {selectedNotice.tags.map((tag, i) => (
                        <span key={i} className="px-2.5 py-1 rounded-md bg-zinc-900 border border-white/5 text-xs text-zinc-400">
                          #{tag.trim()}
                        </span>
                      ))}
                    </div>
              ...
              <div className="flex items-center gap-6 border-y border-white/5 py-4 mb-8">
                <button onClick={(e) => handleLike(selectedNotice._id, e)} className="group flex items-center gap-2">
                  <div className={`p-2 rounded-full transition-colors ${selectedNotice.likes?.includes(currentUser?._id) ? 'bg-red-500/10' : 'bg-white/5 group-hover:bg-red-500/10'}`}>
                    <Heart size={20} className={selectedNotice.likes?.includes(currentUser?._id) ? 'fill-red-500 text-red-500' : 'text-zinc-400 group-hover:text-red-400'} />
                  </div>
                  <span className="font-bold text-white">{selectedNotice.likes?.length || 0}</span>
                </button>
                <div className="flex items-center gap-2">
                  <div className="p-2 rounded-full bg-white/5">
                    <MessageCircle size={20} className="text-zinc-400" />
                  </div>
                  <span className="font-bold text-white">{selectedNotice.comments?.length || 0}</span>
                </div>
              </div>

              {/* Comments Section */}
              <div className="mt-auto">
                <h4 className="text-white font-bold tracking-tight mb-4">Discussion</h4>
                
                {currentUser && (
                  <div className="flex gap-3 mb-6">
                    <div className="w-8 h-8 rounded-full overflow-hidden shrink-0 border border-white/10 mt-1">
                      {currentUser.avatar ? <img src={currentUser.avatar} alt="" /> : <div className="w-full h-full bg-zinc-800" />}
                    </div>
                    <div className="flex-1 relative">
                      <input
                        ...
                        onKeyDown={(e) => e.key === 'Enter' && handleAddComment(selectedNotice._id)}
                ...
                <div className="space-y-4 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
                  {selectedNotice.comments?.length === 0 ? (
                    <p className="text-sm text-zinc-500 italic text-center py-4">No comments yet. Start the conversation!</p>
                  ) : (
                      ...
                      <div key={comment._id} className="flex gap-3">
                        ...
                        <div className="flex-1">
                          <div className="bg-zinc-900 border border-white/5 rounded-2xl rounded-tl-none px-4 py-3">
                            <p className="text-xs font-bold text-white mb-1">{comment.userName}</p>
                            <p className="text-sm text-zinc-300">{comment.text}</p>
                          </div>
                          <p className="text-[10px] text-zinc-600 mt-1 ml-2">{formatDate(comment.createdAt)}</p>
                        </div>
                      </div>
                    ))
                  )}
          ...
          <div className="w-full max-w-sm bg-zinc-950 border border-white/10 rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 md:p-8 text-center flex flex-col items-center">
              ...
              <p className="text-zinc-400 mb-8 text-sm">
                This action cannot be undone. This will permanently remove your notice from the timeline.
              </p>
