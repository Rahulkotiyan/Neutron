              ...
              <div className="flex items-start justify-between">
                ...
                <button
                  ...
                  disabled={!token || starring}
                   className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-bold transition-all duration-200 shrink-0 active:scale-95 min-h-[44px] ${
                     starred
               ...
               <div className="flex items-center gap-2 mt-auto pt-1">
                 <a
                   ...
                   className="inline-flex items-center gap-1.5 px-4 py-2 bg-red-600 hover:bg-red-500 text-white text-xs font-bold rounded-lg transition-all ml-auto shrink-0 active:scale-95 min-h-[44px]"
                >
          ...
          <div className="p-5 flex flex-col h-full">
            ...
            <div className="flex items-start justify-between mb-3">
              ...
              <button
                onClick={handleStar}
                disabled={!token || starring}
                className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-bold transition-all duration-200 active:scale-95 min-h-[44px] ${
                  starred
                    ? "bg-white/20 text-white"
                    : "bg-white/5 text-zinc-500 hover:text-zinc-300 hover:bg-white/10"
                } ${!token ? "opacity-40 cursor-not-allowed" : "cursor-pointer"}`}
            ...
            {tagList.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mb-2">
                {tagList.map((tag, i) => (
                  <span key={i} className="inline-block px-2.5 py-1 bg-white/10 text-zinc-400 text-[10px] md:text-xs font-bold rounded-full truncate max-w-[120px]">
                    {tag}
                  </span>
                ))}
              </div>
            ...
            <div className="flex items-center gap-2 mt-auto">
              {isLongDesc && (
                <button
                  ...
                  className="flex items-center justify-center w-6 h-6 rounded-lg bg-white/5 hover:bg-white/10 text-zinc-500 hover:text-zinc-300 transition-all active:scale-95 min-h-[44px]"
                >
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 px-4 py-2 bg-white hover:bg-zinc-200 text-black text-xs font-bold rounded-lg transition-all duration-200 ml-auto shrink-0 active:scale-95 min-h-[44px]"
              >
                Open
