  ...
  return createPortal(
    <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-zinc-900 border border-white/10 rounded-2xl p-6 max-w-sm w-full shadow-2xl">
        <h3 className={`text-base sm:text-lg md:text-lg font-bold mb-2 ${type === 'error' ? 'text-red-400' : 'text-white'}`}>{title}</h3>
        <p className="text-zinc-300 text-xs sm:text-sm md:text-sm mb-6">{message}</p>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-black/60 backdrop-blur-2xl sticky top-0 z-40 border-b border-white/5">
          <div className="flex items-center gap-4">
            ...
            <h3 className="text-base sm:text-lg md:text-lg font-bold text-white tracking-tight">Post your reply</h3>
          </div>
          <button 
        ...
        <div className="px-4 sm:px-6 pt-4 sm:pt-6 pb-4 bg-[#050505]">
          ...
          <div className="flex gap-3 sm:gap-4 mt-2 min-h-[140px] sm:min-h-[160px]">
            ...
            <div className="flex-1 flex flex-col">
              ...
              {scheduledAt && (
                <div className="mt-3 flex items-center gap-2 px-3 py-1.5 bg-orange-400/10 text-orange-400 rounded-full border border-orange-400/20 w-fit text-[0.65rem] sm:text-xs md:text-xs font-bold animate-in fade-in slide-in-from-left-2">
...
const CharacterCounter = ({ current, max }) => {
  ...
  return (
    <div className="flex items-center gap-3">
      <div className={`text-[0.65rem] sm:text-xs md:text-xs font-medium ${isOverLimit ? 'text-red-500' : isNearLimit ? 'text-yellow-500' : 'text-zinc-500'}`}>
        {current}/{max}
      </div>
