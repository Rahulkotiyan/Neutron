...
async function processPdfToWatermarkedImages(pdfFilePath, user, college) {
  const tempOutputPrefix = path.join(os.tmpdir(), `neutron-pdf-page-${Date.now()}`);
  const watermarkedImageBuffers = [];
  const imageUrls = [];
  const publicIds = [];

  try {
    // 1. Convert PDF pages to temporary images using ImageMagick 'convert'
    // -density controls DPI (higher means larger images, better quality)
    // -quality controls JPEG compression (if output is .jpg)
    // For PNG, remove -quality
    const convertArgs = [
    ...
    await new Promise((resolve, reject) => {
      const convert = spawn('convert', convertArgs); // Make sure 'convert' is in your PATH
      ...
      convert.on('close', (code) => {
        if (code === 0) resolve();
        else reject(new Error(`ImageMagick 'convert' exited with code ${code}`));
      });
      convert.on('error', (err) => {
        console.error("Failed to start ImageMagick 'convert':", err);
        reject(new Error("ImageMagick 'convert' command not found or failed to execute. Is ImageMagick installed and in PATH?"));
      });
    });
    ...
    while (true) {
      ...
      try {
        ...
        // Create a blank SVG for the watermark
        const svgWatermark = `
          <svg width="${2000}" height="${2000}">
            <text x="50%" y="50%" font-family="Arial" font-size="50" fill="#000000" fill-opacity="0.2"
                  text-anchor="middle" dominant-baseline="middle" transform="rotate(-45 1000 1000)">
              ${watermarkText}
            </text>
            <text x="50%" y="25%" font-family="Arial" font-size="50" fill="#000000" fill-opacity="0.2"
                  text-anchor="middle" dominant-baseline="middle" transform="rotate(-45 1000 1000)">
              ${watermarkText}
            </text>
            <text x="50%" y="75%" font-family="Arial" font-size="50" fill="#000000" fill-opacity="0.2"
                  text-anchor="middle" dominant-baseline="middle" transform="rotate(-45 1000 1000)">
              ${watermarkText}
            </text>
          </svg>
        ...
        if (err.code === 'ENOENT') { // File not found, means no more pages
          break;
        }
    ...
    if (imageUrls.length === 0) {
      throw new Error("No images were generated from the PDF. PDF might be empty or corrupt.");
    }
    ...
    try {
      ...
      console.error(`Failed to delete temporary PDF file ${pdfFilePath}: ${err}`);
    }
  }
}
...
router.post('/add', verifyToken, upload.single('file'), async (req, res) => {
  try {
    ...
    if (mimetype === 'application/pdf') {
      ...
      const result = await processPdfToWatermarkedImages(tempFilePath, req.user, req.user.college);
    ...
    if (imageUrls.length === 0) {
      return res.status(400).json({ error: "No viewable content could be generated from the uploaded file." });
    }
    ...
    res.status(500).json({ error: "Failed to upload and process resource: " + err.message });
  } finally {
    ...
    if (req.file && await fs.access(req.file.path).then(() => true).catch(() => false)) {
      await fs.unlink(req.file.path).catch(e => console.error(`Failed to delete multer temp file ${req.file.path}:`, e));
    }
  }
});
...
router.delete('/:id', verifyToken, async (req, res) => {
  try {
    ...
    if (resource.publicIds && resource.publicIds.length > 0) {
      await Promise.all(resource.publicIds.map(async (publicId) => {
        try {
          await cloudinary.uploader.destroy(publicId, { resource_type: 'image' });
        } catch (cloudErr) {
          console.error(`Cloudinary delete warning for publicId ${publicId}:`, cloudErr);
        }
      }));
    }
